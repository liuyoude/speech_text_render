...
- [ ] **1.1.1** 修复 `base_extractor.py` L21: `extract()` 返回类型标注从 `Dict` 改为 `List[Dict]`
- [ ] **1.1.2** 修复 `control_text_builder.py` L40: `generate()` 返回类型标注从 `List[Dict]` 改为 `Dict[int, str]`
- [ ] **1.1.3** 修复 `emotion_extractor.py` L39: `self.sentence_level = "True"` 改为 `self.sentence_level = True`
- [ ] **1.1.4** 修复 `control_text_builder.py` L55: 控制符合并逻辑，改用列表收集再统一格式化，避免字符串裁剪风险

> 完成后检查: 运行 `python core/control_text_builder.py` 确认无报错

### 1.2 分段粒度重构: paragraph/sentence → word/clause/sentence

- [ ] **1.2.1** `text_normalizer.py`: 拆分 `SENTENCE_END_CHARS` 为两个集合
  - `CLAUSE_END_CHARS`: `。！？.!?` + `，,；;`（含句终符，clause 在任何标点处切分）
  - `SENTENCE_END_CHARS`: 仅 `。！？.!?`（sentence 只在句终符切分）
  - 移除 `、`（顿号，过碎）和 `…`（省略号，非边界）
- [ ] **1.2.2** `audio_aligner.py` — `TimeSegment`: type 改为三级 `word` / `clause` / `sentence`，移除 `paragraph`
- [ ] **1.2.3** `audio_aligner.py` — `_merge_to_sentences` 重命名为 `_merge_segments`：
  - 遇到 `CLAUSE_END_CHARS` 中的标点 → 生成一个 clause 级 segment
  - 遇到 `SENTENCE_END_CHARS` 中的标点 → 同时生成一个 sentence 级 segment
  - 移除 paragraph 级 segment 的生成
- [ ] **1.2.4** `audio_aligner.py` — `WhisperAligner._convert_format`: 同步更新，输出 clause/sentence 替代原 sentence/paragraph
- [ ] **1.2.5** `audio_aligner.py` — `MFAAligner._merge_to_sentences`: 同步更新，同 1.2.3 逻辑
- [ ] **1.2.6** `audio_aligner.py` — `SegmentFixer.fix_segments`: 将 `seg.type == "sentence"` 改为 `seg.type == "clause"`，移除 paragraph 相关逻辑
- [ ] **1.2.7** `audio_aligner.py` — `plot_alignment`: 更新可视化中的 type 引用和颜色映射
- [ ] **1.2.8** 各提取器适配: 检查 `speed_extractor.py` / `volume_extractor.py` / `pause_extractor.py` / `emotion_extractor.py` 中遍历 segment 的 type 过滤条件，改为使用正确的 clause 或 sentence 级别
- [ ] **1.2.9** `control_text_builder.py` — `ControlBuilder.build()`: 更新 `seg.type == 'word'` 的拼接逻辑，确保 clause/sentence 级 segment 不参与文本拼接
- [ ] **1.2.10** 在 examples 音频上运行完整 pipeline，验证三级 segment 正确生成，控制符输出格式正常

> 完成后检查: 输出中 clause 在逗号处切分，sentence 在句号处切分，无 paragraph 级 segment

### 1.3 工程规范化

- [ ] **1.3.1** 移除所有文件中的 `sys.path.append`，确认 `pyproject.toml` 配置正确，使用 `pip install -e .` 作为唯一导入方式
- [ ] **1.3.2** 引入 `logging` 模块，替换 `control_text_builder.py` L48 等处的 `print()` 调试输出
- [ ] **1.3.3** 抽取 `SegmentFixer._calculate_perceptual_energy` 和 `VolumeExtractor._calculate_perceptual_energy` 的重复代码为 `core/utils/audio_utils.py` 公共函数

> 完成后检查: `pip install -e . && python -c "from core.feature_extractor import *"` 正常导入

### 1.4 对齐层升级: 集成 Qwen3（先兼容共存）

- [ ] **1.4.1** 安装 qwen-asr: `pip install qwen-asr`，更新 `requirements.txt` 和 `pyproject.toml` 添加 `qwen-asr`
- [ ] **1.4.2** 在 examples 音频上测试 Qwen3-ASR 转录效果（中文 + 英文），记录 WER
- [ ] **1.4.3** 在 examples 音频上测试 Qwen3-ForcedAligner 对齐效果，与当前 Whisper/MFA 逐条对比时间戳精度
- [ ] **1.4.4** 实现 `Qwen3Aligner` 类，封装 Qwen3-ForcedAligner，输出 `List[TimeSegment]`（word/clause/sentence 三级）
- [ ] **1.4.5** 实现 `Qwen3ASR` 类，封装 Qwen3-ASR，支持转录 + 语言检测
- [ ] **1.4.6** 在 `SpeechTextAligner` 中新增 `aligner_backend` 参数（`"qwen3"` / `"whisper_mfa"`），默认 `"qwen3"`，保留原有 Whisper/MFA 作为 fallback
- [ ] **1.4.7** 保留 `SegmentFixer` 作为可选后处理步骤
- [ ] **1.4.8** 在全部 examples 音频上对比 Qwen3 与 Whisper/MFA 的对齐结果，记录详细对比报告

> 完成后检查: 两种 backend 都能正常运行，Qwen3 效果不劣于 Whisper/MFA
>
> **决策点**: 如果 Qwen3 在所有测试音频上效果达标，后续阶段默认使用 Qwen3。如果部分场景不如 MFA，记录具体 case 并保留双后端。

### 1.5 清理旧依赖（确认 Qwen3 效果后执行）

- [ ] **1.5.1** 移除 Whisper 相关代码: `WhisperAligner`, `_convert_format`, `_correct_text_Errors`, `_dynamic_align_words`
- [ ] **1.5.2** 移除 MFA 相关代码: `MFAAligner`, `mfa_model/` 目录
- [ ] **1.5.3** 更新 `requirements.txt` 和 `pyproject.toml`: 移除 `openai-whisper`
- [ ] **1.5.4** 更新 `setup.sh`: 移除 MFA 和 Whisper 相关安装步骤
- [ ] **1.5.5** 简化 `SpeechTextAligner`，移除 `aligner_backend` 切换逻辑和语言分支

> **如果 1.4 决策点结论是保留双后端**: 跳过此节，Whisper/MFA 作为可选后端继续保留。
