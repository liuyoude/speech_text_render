...
- [ ] **1.3.1** 安装 qwen-asr: `pip install qwen-asr`，更新 `requirements.txt` 和 `pyproject.toml` 添加 `qwen-asr`
- [ ] **1.3.2** 在 examples 音频上测试 Qwen3-ASR 转录效果（中文 + 英文），记录 WER
- [ ] **1.3.3** 在 examples 音频上测试 Qwen3-ForcedAligner 对齐效果，与当前 Whisper/MFA 逐条对比时间戳精度
- [ ] **1.3.4** 实现 `Qwen3Aligner` 类，封装 Qwen3-ForcedAligner，输出 `List[TimeSegment]`（暂用现有 word/sentence/paragraph 格式，粒度重构在 1.5 统一做）
- [ ] **1.3.5** 实现 `Qwen3ASR` 类，封装 Qwen3-ASR，支持转录 + 语言检测
- [ ] **1.3.6** 在 `SpeechTextAligner` 中新增 `aligner_backend` 参数（`"qwen3"` / `"whisper_mfa"`），默认 `"qwen3"`，保留原有 Whisper/MFA 作为 fallback
- [ ] **1.3.7** 保留 `SegmentFixer` 作为可选后处理步骤
- [ ] **1.3.8** 在全部 examples 音频上对比 Qwen3 与 Whisper/MFA 的对齐结果，记录详细对比报告
...
- [ ] **1.4.1** 移除 Whisper 相关代码: `WhisperAligner`, `_convert_format`, `_correct_text_Errors`, `_dynamic_align_words`
- [ ] **1.4.2** 移除 MFA 相关代码: `MFAAligner`, `mfa_model/` 目录
- [ ] **1.4.3** 更新 `requirements.txt` 和 `pyproject.toml`: 移除 `openai-whisper`
- [ ] **1.4.4** 更新 `setup.sh`: 移除 MFA 和 Whisper 相关安装步骤
- [ ] **1.4.5** 简化 `SpeechTextAligner`，移除 `aligner_backend` 切换逻辑和语言分支

> **如果 1.3 决策点结论是保留双后端**: 跳过此节，Whisper/MFA 作为可选后端继续保留。
...
- [ ] **1.5.1** `text_normalizer.py`: 拆分 `SENTENCE_END_CHARS` 为两个集合
  - `CLAUSE_END_CHARS`: `。！？.!?` + `，,；;`（含句终符，clause 在任何标点处切分）
  - `SENTENCE_END_CHARS`: 仅 `。！？.!?`（sentence 只在句终符切分）
  - 移除 `、`（顿号，过碎）和 `…`（省略号，非边界）
- [ ] **1.5.2** `audio_aligner.py` — `TimeSegment`: type 改为三级 `word` / `clause` / `sentence`，移除 `paragraph`
- [ ] **1.5.3** `Qwen3Aligner` 的 segment 合并逻辑：
...
- [ ] **1.5.4** `SegmentFixer.fix_segments`: 将 `seg.type == "sentence"` 改为 `seg.type == "clause"`，移除 paragraph 相关逻辑
- [ ] **1.5.5** `plot_alignment`: 更新可视化中的 type 引用和颜色映射
- [ ] **1.5.6** 各提取器适配: 检查 `speed_extractor.py` / `volume_extractor.py` / `pause_extractor.py` / `emotion_extractor.py` 中遍历 segment 的 type 过滤条件，改为使用正确的 clause 或 sentence 级别
- [ ] **1.5.7** `control_text_builder.py` — `ControlBuilder.build()`: 更新 `seg.type == 'word'` 的拼接逻辑，确保 clause/sentence 级 segment 不参与文本拼接
- [ ] **1.5.8** 在 examples 音频上运行完整 pipeline，验证三级 segment 正确生成，控制符输出格式正常
